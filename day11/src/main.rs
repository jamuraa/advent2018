...
fn power_grid(top_left_x: i32, top_left_y: i32, size: i32, serial: i32, memo: &mut HashMap<(i32, i32, i32), i32>) -> i32 {
    ...
    let result = match size {
        ...
        _ => {
            ...
            let br = memo.get(&(top_left_x+1, top_left_y+1, size - 1)).unwrap();
            let tr = power_lvl(top_left_x + size - 1, top_left_y, serial);
            let bl = power_lvl(top_left_x, top_left_y + size - 1, serial);
            let center = memo.get(&(top_left_x + 1, top_left_y + 1, size - 2)).unwrap();
            tl + br + tr + bl - center
        }
    };
    ...
    for topleft_x in 1..(302 - size) {
        for topleft_y in 1..(302 - size) {
            ...
            if grid > max_grid {
                ...
                max_size = size;
                max_grid = grid;
                println!("Grid at {},{} size {} has {} power", topleft_x, topleft_y, size, grid);
            }
        }
    }
    }

    println!("Grid at {},{} size {} has {} power", max_x, max_y, max_size, max_grid);
}
